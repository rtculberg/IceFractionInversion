...
% sigma_h - rms roughness of surface in meters (scalar double)
...
function [x_keep, L_keep, count, reflectivity, accept] = mcmc_gauss(data,x0,xstep,xbnds,sigma,sigma_h,Niter,coefficients,depth,filename)
    

    h = 435;         % average flight altitude
    n_ice = 1.78;    % index of refraction in solid ice
    fc = 300e6;      % radar center frequency
    del_az = 2.5;    % focused azimuth resolution
    ...
    % Calculate the remaining area occupied by solid ice and assign a base
    % reflectivity of -67 dB
    A_rem = 1 - sum(A);
    A = [A A_rem];
    R = [R 4.47e-4];   % Equivalent to base reflectivity of -67 dB
    % Calculate a random phase screen based on the surface roughness metric
    phase_error = ((4*pi*sigma_h)/lambda)*(n_ice-1)*randn(size(R));
    % Apply phase errors to each firn patch and calculate the effective 
    % reflectivity of radar footprint as the coherent, area weighted sum of 
    % all the reflectors within the footprint
    reflectivity(1) = 10*log10(abs(sum(A.*R.*exp(1i*phase_error))).^2);
    ...
    for k = 1:Niter
        ...
        if flag == 0  % if proposed model is within the bounds,
            ...
            R = [R 4.47e-4]; 
            phase_error = ((4*pi*sigma_h)/lambda)*(n_ice-1)*randn(size(R));
            dprop = 10*log10(abs(sum(A.*R.*exp(1i*phase_error))).^2);
